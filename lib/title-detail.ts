...
function trailerFrom(detail: TmdbDetail) {
  const trailer =
    detail.videos?.results?.find((video) => video.site === "YouTube" && video.type === "Trailer" && video.official) ||
...
async function animeForDetail(detail: TmdbDetail): Promise<AniListMedia | null> {
  const matches = await searchAnime(titleFrom(detail));
...
export function getTitleDetail(type: TitleType, id: number) {
  return cached(`title:v1:${type}:${id}`, 21600, () => buildTitleDetail(type, id));
}
